...
function startProgram (prog, exec) {
    ...
    crash_queued = false;
    var child = exports.child = spawn(exec, ['-r', path.join(__dirname, 'proxy-hot-reload.js')].concat(prog), {stdio: 'inherit'});
...
var findAllWatchFiles = function(dir, callback) {
    ...
    if (ignoredPaths[dir])
        return;
    try {
        var stats = fs[ignoreSymLinks ? 'lstatSync' : 'statSync'](dir)
